...
void ofApp::setup(){
    ...
    for (int i = 0; i < particleNum; i++) {
        ofVec3f position = ofVec3f(ofRandom(width), ofRandom(height), ofRandom(-10,10));
    ...
    for (int j = 0; j < height; j++) {
        for (int i = 0; i < width; i++) {
            ...
            float noiseZ = ofMap(j, 0, width, 0, scaleZ);
            int noiseVal = ofNoise(noiseX, noiseY, noiseZ) * 255;
    ...
    gui.add(scaleZ.setup("scale z", 5.0, 1.0, 10.0));
}
