...
void Particles::resetForce(){
    for(int i = 0; i < positions.size(); i++){
        forces[i] = ofVec3f(0, 0);
    }
}

void Particles::addForce(int count, ofVec3f force){
    forces[count] += force;
}
...
void Particles::draw(){
    mesh.clear();
    for (int i = 0; i < positions.size(); i++) {
        mesh.addVertex(ofVec3f(positions[i].x, positions[i].y, positions[i].z));
        mesh.addColor(ofFloatColor(1.0,1.0,1.0));
    }
    mesh.draw();
}

void Particles::addParticle(ofVec3f _position, ofVec3f _velocity, ofColor _color){
